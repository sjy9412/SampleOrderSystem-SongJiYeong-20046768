...
### REVIEW - 코드 리뷰 + 사용자 승인 후 Commit

GREEN 완료 후, 구현한 코드를 다음 기준으로 리뷰하라:

**Step 1: Plan 준수 확인**

- Plan.md에서 정의한 goal만 구현했는가?
- Plan 외의 기능이 추가되지 않았는가?
- 과도한 설계(YAGNI 위반)는 없는가?

**Step 2: 리팩토링 필요 여부 판단**
...
리팩토링 시 테스트는 그린으로 유지하라. 동작을 추가하지 말라.

**Step 3: 사용자 리뷰 대기 후 Commit**

리뷰 결과를 사용자에게 보여주고 승인을 받아라. 승인이 완료되면 코드를 커밋한다.
