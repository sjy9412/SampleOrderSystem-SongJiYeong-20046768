...
TDD는 사람 파트너가 루프 안에 있어야 한다. 각 단계마다 사용자가 검토하고 승인한다.

```dot
digraph tdd_cycle {
    rankdir=LR;
    red [label="RED\n목표 설정 + Plan.md", shape=box, style=filled, fillcolor="#ffcccc"];
    review_plan [label="사용자\nPlan 검토", shape=diamond];
    commit_plan [label="Plan.md\nCommit", shape=box, style=filled, fillcolor="#ffcccc"];
    green [label="GREEN\ngoal 달성 구현", shape=box, style=filled, fillcolor="#ccffcc"];
    verify_green [label="테스트\n통과 확인", shape=diamond];
    review [label="REVIEW\n코드 리뷰", shape=box, style=filled, fillcolor="#ccccff"];
    commit_code [label="코드\nCommit", shape=box, style=filled, fillcolor="#ccccff"];
    next [label="다음", shape=ellipse];

    red -> review_plan;
    review_plan -> commit_plan [label="승인"];
    review_plan -> red [label="수정 요청"];
    commit_plan -> green;
    green -> verify_green;
    verify_green -> review [label="통과"];
    verify_green -> green [label="실패"];
    review -> commit_code [label="승인"];
    review -> green [label="리팩토링\n요청"];
    commit_code -> next;
    next -> red;
}
```

### RED - 목표 설정 + Plan.md 작성

**Step 1: 목표 정확히 설정**

다음 테스트 케이스로 무엇을 검증할지 명확히 정의하라.

**Step 2: Plan.md 생성 (tmp/ 경로)**

`tmp/Plan.md`를 생성하여 아래를 기록하라:
- 이번 사이클의 목표 (goal)
- 작성할 테스트 코드
- 예상 실패 이유
- 구현 방향 (최소한의 코드)
...
Plan.md를 사용자에게 보여주고 검토를 요청하라. 사용자가 승인할 때까지 구현 코드를 작성하지 말라.

**Step 4: Plan.md Commit**

검토가 완료되면 Plan.md를 커밋한다.
...
### GREEN - RED의 goal을 달성하는 최소한의 코드

**Plan.md의 goal만을 달성하라.** Plan 외의 기능을 추가하지 말라.
...
### REVIEW - 사용자 코드 리뷰 + 승인 후 Commit

GREEN 완료 후, 변경된 파일 목록과 테스트 통과 결과를 사용자에게 제시하고 리뷰를 요청하라.

**Claude가 할 일**

- 변경된 파일과 주요 변경 내용을 간결하게 요약한다.
- 테스트 통과 결과를 보여준다.
- 사용자의 리뷰와 승인을 기다린다. 사용자가 승인할 때까지 커밋하지 않는다.

**사용자가 할 일**

사용자가 직접 다음을 검토한다:
- Plan.md에서 정의한 goal만 구현되었는가?
- Plan 외의 기능이 추가되지 않았는가?
- 과도한 설계(YAGNI 위반)는 없는가?
- 리팩토링이 필요한 부분(중복, 이름, 헬퍼 추출)이 있는가?

**리팩토링 요청 시**

사용자가 리팩토링을 요청하면 테스트를 그린으로 유지하면서 수정한 뒤 다시 사용자 리뷰를 요청하라. 동작을 추가하지 말라.

**Step: 사용자 승인 후 Commit**
